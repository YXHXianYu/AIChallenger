	...
	*/
	void AI(Character &t, map<string, int> &mem) {
		...
		if(mem.count("init") == 0) {
			...
			register int i, j;
			// 学习模式
			int run_mode = 2; // 1:训练模式; 2:测试模式(成果检测); 3:纯随机模式 
			if(run_mode == 1) {
				mem["epsilon"] = 50; // 百分值 
				mem["alpha"] = 50;
				mem["gamma"] = 100;
			...
			// 视野范围对应表格
			mem["mx"+TS(1)] = 0; mem["my"+TS(1)] = 0;
			for(i = 2; i <= 16; i++) {mem["mx"+TS(i)] = (i+1)%3-1; mem["my"+TS(i)] = (i+1)/3-7;}
			for(i = 17; i <= 29; i++) {mem["mx"+TS(i)] = i - 23; mem["my"+TS(i)] = -1;}
			for(i = 30; i <= 35; i++) {mem["mx"+TS(i)] = i - 36; mem["my"+TS(i)] = 0;}
			for(i = 36; i <= 41; i++) {mem["mx"+TS(i)] = i - 35; mem["my"+TS(i)] = 0;}
			for(i = 42; i <= 54; i++) {mem["mx"+TS(i)] = i - 48; mem["my"+TS(i)] = 1;}
			for(i = 55; i <= 69; i++) {mem["mx"+TS(i)] = (i-1)%3-1; mem["my"+TS(i)] = (i-1)/3-16;}
			for(i = 1; i <= 69; i++)
				mem["to_id"+TS(mem["mx"+TS(i)]+6)+","+TS(mem["my"+TS(i)]+6)] = i; 
			// R 表格
			for(i = 1; i <= 69; i++)
				mem["Rm"+TS(i)+","+TS(0)] = 0,
				mem["Ra"+TS(i)+","+TS(0)] = 0;
		...
		for(int i = 1; i <= player_sum; i++)
			if(i != iNumber && player_hp[i] > 0) { // 玩家不是自己 并且 存活 则 选定为目标 
				...
				int dx = player_x[i] - iX + 6,
					dy = player_y[i] - iY + 6;
				if(dx < 0 || dy < 0 || dx > 12 || dy > 12) continue; // 超出视野距离 
				//printf(".\n");
